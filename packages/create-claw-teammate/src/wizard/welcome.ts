...
export async function welcomeStep(): Promise<void> {
  ...
  console.log(
    chalk.bold("  Deploy your own AI teammate on your cloud\n"),
  );
  console.log(
    chalk.dim("  Bring your own API keys. Own your data. Run on your infra.\n"),
  );
  ...
  if (major < 22) {
    console.log(
      chalk.red(`  ✗ Node.js >= 22 required (found v${nodeVersion})`),
    );
    console.log(
      chalk.dim("    Install: https://nodejs.org/en/download\n"),
    );
    process.exit(1);
  }
  ...
  if (!authResult.stdout.trim()) {
    ...
    console.log(
      chalk.dim("    Run: gcloud auth login\n"),
    );
    process.exit(1);
  }
  console.log(
    chalk.green(`  ✓ gcloud authenticated as ${authResult.stdout.trim()}`),
  );
