...
import { gcloud, gcloudWithSpinner, run, runWithSpinner } from "../utils/shell.js";
...
export class GcpDeployer implements CloudDeployer {
  ...
  async deploy(config: DeployConfig): Promise<void> {
    ...
    console.log(
      `    1. Check status:  gcloud compute ssh ${this.vmName} --zone=${this.zone} -- openclaw status`,
    );
    console.log(
      `    2. SSH tunnel:    gcloud compute ssh ${this.vmName} --zone=${this.zone} -- -L 18789:localhost:18789`,
    );
    console.log(
  ...
  dryRun(config: DeployConfig): void {
    ...
    console.log(
      `  gcloud services enable compute.googleapis.com secretmanager.googleapis.com drive.googleapis.com --project=${this.project}\n`,
    );

    console.log(chalk.dim("  # Service Account"));
    console.log(
      `  gcloud iam service-accounts create openclaw-sa --display-name="OpenClaw SA" --project=${this.project}`,
    );
    console.log(
      `  gcloud projects add-iam-policy-binding ${this.project} --member=serviceAccount:${saEmail} --role=roles/secretmanager.secretAccessor\n`,
    );

    console.log(chalk.dim("  # Secrets"));
    for (const secretName of Object.keys(config.apiKeys)) {
      console.log(
        `  gcloud secrets create ${secretName} --project=${this.project}`,
      );
    }
    console.log();
    ...
    console.log(
      `  gcloud compute firewall-rules create allow-iap-ssh --project=${this.project} --rules=tcp:22 --source-ranges=35.235.240.0/20`,
    );
    console.log(
      `  gcloud compute firewall-rules create deny-all-ingress --project=${this.project} --action=DENY --rules=all\n`,
    );

    console.log(chalk.dim("  # VM"));
    console.log(
      `  gcloud compute instances create ${this.vmName} --zone=${this.zone} --machine-type=${config.gcp.machineType} --image-family=debian-12 --no-address\n`,
    );
    ...
    for (const s of enabledSkills) {
      ...
      if (meta?.cron) {
        console.log(
          `  openclaw cron add --name '${s}' --cron '${meta.cron}' --message '/${s}'`,
        );
      }
    }
  ...
  private async enableApis(): Promise<void> {
  ...
  private async ensureServiceAccount(saEmail: string): Promise<void> {
  ...
  private async writeStartupScript(): Promise<string> {
  ...
  private async createVm(saEmail: string, startupFile: string): Promise<void> {
  ...
  private async configurePlugins(): Promise<void> {
    ...
    for (const pluginId of enabledPlugins) {
      ...
      if (
        (pluginId === "image-gen" || pluginId === "video-gen") &&
...
`.trim();
  ...
  private getPluginSourcePath(pluginId: string): string {
    return join(this.getPackageRoot(), "plugin-files", pluginId);
  }

  private getSkillsSourcePath(): string {
