...
export async function confirmStep(config) {
    ...
    if (config.branding.systemPrompt) {
        console.log(`    System prompt: ${config.branding.systemPrompt.substring(0, 60)}...`);
    }
    // Plugins
    ...
    console.log(`    GCP VM: ${costMap[config.gcp.machineType] ?? "varies"} (compute only)`);
